...
In this project, I worked on the Bayan bilingual NLP service as a student in the SDA-AIE-211 course.
I followed the labs step by step, starting with Arabic and English text preprocessing and tokenization, then moving to Transformer attention, text classification, NER, extractive QA, Arabic dialect handling, semantic search, model evaluation, and finally model optimisation and FastAPI serving.

The main goal of my work was to build and connect the different NLP components into one practical bilingual service, while testing the implementation and recording the results and decisions made during the labs.
...
As a student, I developed the Bayan project through a series of practical labs. Instead of treating each lab as a separate task, I reused the work from previous labs until it became one complete NLP pipeline.

My work covered:

Preparing and cleaning Arabic and English text.

Protecting personal information (PII) before using the text with models.

Comparing tokenizers and choosing one based on measured results.

Implementing Transformer attention and checking it against PyTorch.

Building topic classification, NER, and extractive QA.

Adding Arabic normalisation and dialect-aware processing.

Building bilingual semantic search using FAISS and re-ranking.

Evaluating the models using metrics, slices, confidence intervals, and behavioural tests.

Optimising inference with ONNX and INT8.

Connecting the final pipeline to a FastAPI service.
...
I followed the repository as a step-by-step practical project. For each lab, I first opened the files mentioned in the instructions, ran the required test or script, implemented the missing parts, ran the checks again, recorded my results, and then committed my work to GitHub.

Step 1 — Set up the project

I used Python 3.12 and created a virtual environment.
...
The expected environment result is:

ALL GOOD

Step 2 — Lab 1: Preprocessing and Tokenisation

First, I inspected the raw Bayan data and looked for issues such as Unicode differences, Tatweel, Arabic/English code-switching, PII, emojis, and HTML remnants.

Then I completed the preprocessing functions and checked them with:

pytest tests/test_preprocessing.py -q
pytest tests/test_pii_recall.py -q

After that, I implemented sentence segmentation, ran the tokenizer audit, compared the four supplied tokenizers, and recorded the measured results and final tokenizer decision.

python notebooks/01_tokenizer_audit.py

Step 3 — Lab 2: Transformer Attention

I implemented scaled dot-product attention and Multi-Head Attention. I then compared the implementation with the PyTorch reference and checked masking behaviour.

pytest tests/test_attention.py -q
python scripts/parameter_audit.py
python notebooks/02_transformer_anatomy.py

I also implemented causal masking and inspected attention maps to identify issues such as attention paid to padding tokens.

Step 4 — Lab 3: Classification, NER, and QA

For topic classification, I first created the TF-IDF baseline, then built leakage-safe grouped data splits and fine-tuned the Transformer classifier.

python scripts/tfidf_baseline.py
pytest tests/test_model_data.py -q
python scripts/train_classifier.py

For NER, I implemented BIO label alignment, trained the NER model, and checked the required alignment tests.

pytest tests/test_ner_alignment.py -q
python scripts/train_ner.py

For extractive QA, I implemented span selection and tested both answerable and unanswerable questions.

pytest tests/test_qa.py -q
python scripts/qa_smoke.py

Step 5 — Lab 4: Arabic Processing

I added Arabic-specific normalisation, checked the dialect distribution, implemented clitic segmentation, and compared Arabic-focused models using the provided slices.

pytest tests/test_arabic_normalize.py -q
python scripts/dialect_audit.py
python scripts/arabic_bakeoff.py

I recorded the results and used the measured evidence to make the Arabic-model decision.

Step 6 — Lab 5: Bilingual Semantic Search

I built the FAISS search index, kept the index and model versions in a manifest, and implemented the two-stage search process:

Bi-encoder retrieval → candidate retrieval → Cross-encoder re-ranking

Then I evaluated retrieval using Recall@10, MRR@10, cross-lingual performance, and no-answer behaviour.

pytest tests/test_search_contract.py -q
python notebooks/05_retrieval_eval.py

Step 7 — Lab 6: Model Evaluation

I implemented bootstrap confidence intervals, sliced evaluation, and behavioural tests. I also manually reviewed the supplied validation errors and organised them into an error taxonomy.

Finally, I generated the evaluation report and model cards.

pytest tests/test_evaluation.py -q
python scripts/evaluation_report.py

Step 8 — Lab 7: Optimisation and FastAPI

Before optimising, I measured the CPU inference baseline. Then I exported the classifier to ONNX, tested INT8 quantisation, and compared speed and quality against the original model.

python scripts/benchmark_inference.py
python scripts/export_onnx.py

After that, I connected the selected model to the FastAPI service and checked the serving contract.

pytest tests/test_serving_contract.py -q
make serve

The API can be checked through:

GET http://localhost:8000/health

Finally, I ran the HTTP load test:

bash scripts/load_test.sh

Step 9 — Final Capstone

At the end, I integrated the components from Labs 1–7 into the Bayan service instead of creating a new project.

The final service includes endpoints for:
...
Step 10 — GitHub Checkpoint

After completing each lab, I saved my progress using Git:
...
This helped me keep a clear history of the work completed throughout the project.
